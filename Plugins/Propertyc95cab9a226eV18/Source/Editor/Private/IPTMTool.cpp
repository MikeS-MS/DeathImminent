...
void FPTMTool::StartupModule() {
	...
	FLevelEditorModule &LevelEditorModule = FModuleManager::LoadModuleChecked<FLevelEditorModule>("LevelEditor");
	{
		...
		PTM_Extender->AddMenuExtension("ActorTypeTools",EExtensionHook::After,nullptr,FMenuExtensionDelegate::CreateStatic(&FPTMTool::PTM_ExtendMenu));
		LevelEditorModule.GetMenuExtensibilityManager()->AddExtender(PTM_Extender);
		//
		FTM_Extender = MakeShareable(new FExtender());
		FTM_Extender->AddMenuExtension("ActorTypeTools",EExtensionHook::After,nullptr,FMenuExtensionDelegate::CreateStatic(&FPTMTool::FTM_ExtendMenu));
		...
		//
		UToolMenus::Get()->RegisterStartupCallback(FSimpleMulticastDelegate::FDelegate::CreateRaw(this,&FPTMTool::RegisterMenus));
	}
}
...
void FPTMTool::RegisterMenus() {
	FToolMenuOwnerScoped OwnerScoped(this);
	{
		UToolMenu* Menu = UToolMenus::Get()->ExtendMenu("LevelEditor.MainMenu.Tools");
		FToolMenuSection &Section = Menu->AddSection("DataTransfer",LOCTEXT("DataTransfer","DataTransfer"));
		//
		Section.AddEntry(FToolMenuEntry::InitMenuEntry
		(
			"Property Transfer",
			TAttribute<FText>::Create(&Menu_PropertyTransferGetTitle),
			LOCTEXT("PropertyTransferTooltip","Property Transfer tool."),
		...
		(
			"Function Transfer",
			TAttribute<FText>::Create(&Menu_FunctionTransferGetTitle),
			LOCTEXT("FunctionTransferTooltip","Function Transfer tool."),
			FSlateIcon(FPTM_Style::Get().Get()->GetStyleSetName(),"PTMTool_Window.Tab"),
...
FText FPTMTool::Menu_PropertyTransferGetTitle()
{
	return LOCTEXT("PropertyTransferTitle","Property Copy...");
}

FText FPTMTool::Menu_FunctionTransferGetTitle()
{
	return LOCTEXT("FunctionTransferTitle","Function Copy...");
}
