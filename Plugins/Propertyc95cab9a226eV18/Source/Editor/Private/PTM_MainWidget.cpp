...
FReply SPTM_MainWidget::OnClickedApply()
{
	if (!IsEnabled()) return FReply::Handled();
	//
	EAppReturnType::Type Action = FMessageDialog::Open(
		EAppMsgType::YesNo,
		FText::FromString(TEXT(" Copy picked properties from Source to Target Blueprint(s) ?!")),
		FText::FromString("Transfer Properties")
	);
	//
	if (Action != EAppReturnType::Yes) return FReply::Handled();
	if (!HasSourceAndTarget()) return FReply::Handled();
	//
	...
	//
	// --- Single Target ---
	for (const auto& Item : PTM_TARGET_ITEMS)
	{
		if (!Item.IsValid() || Item->GetState() != ESPTM_State::Transfer || !Item->GetPropertyInfo().Property.IsValid())
		{
			continue;
		}
		FPropertyTransferManager::CopyBlueprintProperty(Source.Get(), Target.Get(), Item->GetPropertyInfo().Property->GetFName());
	}
	//
	...
	if (TargetObject)
	{
		...
		if (UBlueprint* Blueprint = FPropertyTransferManager::GetBlueprintFromObject(TargetObject))
		{
			FBlueprintEditorUtils::MarkBlueprintAsStructurallyModified(Blueprint);
			FKismetEditorUtilities::CompileBlueprint(Blueprint);
			Package = Blueprint->GetOutermost();
		}
		else if (UUserDefinedStruct* Struct = Cast<UUserDefinedStruct>(TargetObject))
		{
			FStructureEditorUtils::CompileStructure(Struct);
			Struct->MarkPackageDirty();
			Package = Struct->GetOutermost();
		}
		//
		if (Package)
		{
	...
	for (const TWeakObjectPtr<UObject>& TargetPtr : TargetBatch)
	{
		if (!TargetPtr.IsValid()) continue;
		UObject* EachTarget = TargetPtr.Get();
		//
		for (const auto& Item : PTM_TARGET_ITEMS)
		{
			if (!Item.IsValid() || Item->GetState() != ESPTM_State::Transfer || !Item->GetPropertyInfo().Property.IsValid())
			...
			FPropertyTransferManager::CopyBlueprintProperty(Source.Get(), EachTarget, Item->GetPropertyInfo().Property->GetFName());
		}
		//
		UPackage* Package = nullptr;
		if (UBlueprint* Blueprint = FPropertyTransferManager::GetBlueprintFromObject(EachTarget))
		{
			FBlueprintEditorUtils::MarkBlueprintAsStructurallyModified(Blueprint);
			FKismetEditorUtilities::CompileBlueprint(Blueprint);
			Package = Blueprint->GetOutermost();
		}
		else if (UUserDefinedStruct* Struct = Cast<UUserDefinedStruct>(EachTarget))
		{
			FStructureEditorUtils::CompileStructure(Struct);
			Struct->MarkPackageDirty();
			Package = Struct->GetOutermost();
