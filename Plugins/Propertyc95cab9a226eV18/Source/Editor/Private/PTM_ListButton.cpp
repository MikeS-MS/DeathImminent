...
FText SPTM_ListButton::GetButtonCaption() const {
	...
	//
	FString Guid{};
	FString GuidName = PPInfo.Property->GetDisplayNameText().ToString();
	GuidName.Split(TEXT("_"), &GuidName, &Guid, ESearchCase::IgnoreCase, ESearchDir::FromEnd);
	//
	if (Guid.Len() == 32)
	{
		return FText::FromString(GuidName);
	}
	//
	return PPInfo.Property->GetDisplayNameText();
}
