...
void UFileHelperBPLibrary::ReadConfigKey(const FString& InFilePath, const FString& InSection, const FString& InKey, bool& Success, bool SingleLineArrayRead, UStruct*& OutValue)
{
	checkNoEntry();
}
...
bool UFileHelperBPLibrary::WriteConfigCache(const FString& Filename, const FString& Section, const FString& Key, FProperty* Type, void* Value, bool SingleLineArray)
{
	if (!GConfig)
	{
		return false;
	...
	const FString NormalizedFilePath = FConfigCacheIni::NormalizeConfigIniPath(Filename);
	const FConfigFile* ConfigFile = FindConfigCache(NormalizedFilePath);
	if (!ConfigFile)
	{
		return false;
	}
	...
	{
		GConfig->SetBool(*Section, *Key, *(static_cast<bool*>(Value)), NormalizedFilePath);
	}
	else if (FIntProperty* IntProperty = CastField<FIntProperty>(Type))
	{
		GConfig->SetInt(*Section, *Key, *(static_cast<int32*>(Value)), NormalizedFilePath);
	}
	else if (FStrProperty* StrProperty = CastField<FStrProperty>(Type))
	{
		GConfig->SetString(*Section, *Key, **(static_cast<FString*>(Value)), NormalizedFilePath);
	}
	else if (FFloatProperty* FloatProperty = CastField<FFloatProperty>(Type))
	{
		GConfig->SetFloat(*Section, *Key, *(static_cast<float*>(Value)), NormalizedFilePath);
	}
	else if (FDoubleProperty* DoubleProperty = CastField<FDoubleProperty>(Type))
	{
		GConfig->SetDouble(*Section, *Key, *(static_cast<double*>(Value)), NormalizedFilePath);
	}
	else if (FArrayProperty* ArrayProperty = CastField<FArrayProperty>(Type))
	{
		...
		{
			const TArray<FString>* Arr = (static_cast<TArray<FString>*>(Value));
			if (SingleLineArray)
			{
				GConfig->SetSingleLineArray(*Section, *Key, *Arr, NormalizedFilePath);
			}
			else
			{
				GConfig->SetArray(*Section, *Key, *Arr, NormalizedFilePath);
			}
		}
		else
		{
	...
	{
		GConfig->SetText(*Section, *Key, *(static_cast<FText*>(Value)), NormalizedFilePath);
	}
	else if (const FStructProperty* StructProperty = CastField<FStructProperty>(Type))
	{
		if (StructProperty->Struct->IsNative())
		{
			...
			static const FName ColorType = TEXT("Color");
			...
			{
				GConfig->SetRotator(*Section, *Key, *(static_cast<FRotator*>(Value)), *NormalizedFilePath);
			}
			else if (TypeName == VectorType)
			{
				GConfig->SetVector(*Section, *Key, *(static_cast<FVector*>(Value)), *NormalizedFilePath);
			}
			else if (TypeName == LinearColorType)
			{
				GConfig->SetColor(*Section, *Key, static_cast<FLinearColor*>(Value)->ToFColorSRGB(), *NormalizedFilePath);
			}
			else if (TypeName == ColorType)
			{
				GConfig->SetColor(*Section, *Key, *(static_cast<FColor*>(Value)), *NormalizedFilePath);
			}
			else if (TypeName == Vector4Type)
			{
				GConfig->SetVector4(*Section, *Key, *(static_cast<FVector4*>(Value)), *NormalizedFilePath);
			}
			else if (TypeName == Vector2DType)
			{
				GConfig->SetVector2D(*Section, *Key, *(static_cast<FVector2D*>(Value)), *NormalizedFilePath);
			}
			else
			{
				return false;
...
bool UFileHelperBPLibrary::ReadConfigCache(const FString& Filename, const FString& Section, const FString& Key, FProperty* Type, void* Value, bool SingleLineArray)
{
	if (!GConfig)
	{
		return false;
	...
	if (FBoolProperty* BoolProperty = CastField<FBoolProperty>(Type))
	{
		Success = GConfig->GetBool(*Section, *Key, *(static_cast<bool*>(Value)), NormalizedFilePath);
	}
	else if (FIntProperty* IntProperty = CastField<FIntProperty>(Type))
	{
		Success = GConfig->GetInt(*Section, *Key, *(static_cast<int32*>(Value)), NormalizedFilePath);
	}
	else if (FStrProperty* StrProperty = CastField<FStrProperty>(Type))
	{
		Success = GConfig->GetString(*Section, *Key, *(static_cast<FString*>(Value)), NormalizedFilePath);
	}
	else if (FFloatProperty* FloatProperty = CastField<FFloatProperty>(Type))
	{
		Success = GConfig->GetFloat(*Section, *Key, *(static_cast<float*>(Value)), NormalizedFilePath);
	}
	else if (FDoubleProperty* DoubleProperty = CastField<FDoubleProperty>(Type))
	{
		Success = GConfig->GetDouble(*Section, *Key, *(static_cast<double*>(Value)), NormalizedFilePath);
	}
	else if (const FArrayProperty* ArrayProperty = CastField<FArrayProperty>(Type))
	{
		if (FStrProperty* ArrayInnerProperty = CastField<FStrProperty>(ArrayProperty->Inner))
		{
			...
			if (SingleLineArray)
			{
				Success = (GConfig->GetSingleLineArray(*Section, *Key, *Arr, NormalizedFilePath) != 0);
			}
			else
			{
				Success = (GConfig->GetArray(*Section, *Key, *Arr, NormalizedFilePath) != 0);
			}
		}
	}
	else if (FTextProperty* TextProperty = CastField<FTextProperty>(Type))
	{
		Success = GConfig->GetText(*Section, *Key, *(static_cast<FText*>(Value)), NormalizedFilePath);
	}
	else if (const FStructProperty* StructProperty = CastField<FStructProperty>(Type))
	{
		if (StructProperty->Struct->IsNative())
		{
			...
			if (TypeName == RotatorType)
			{
				Success = GConfig->GetRotator(*Section, *Key, *(static_cast<FRotator*>(Value)), *NormalizedFilePath);
			}
			else if (TypeName == VectorType)
			{
				Success = GConfig->GetVector(*Section, *Key, *(static_cast<FVector*>(Value)), *NormalizedFilePath);
			}
			else if (TypeName == ColorType)
			{
				Success = GConfig->GetColor(*Section, *Key, *(static_cast<FColor*>(Value)), *NormalizedFilePath);
			}
			else if (TypeName == Vector4Type)
			{
				Success = GConfig->GetVector4(*Section, *Key, *(static_cast<FVector4*>(Value)), *NormalizedFilePath);
			}
			else if (TypeName == Vector2DType)
			{
				Success = GConfig->GetVector2D(*Section, *Key, *(static_cast<FVector2D*>(Value)), *NormalizedFilePath);
			}
		}
	}
...
FConfigFile* UFileHelperBPLibrary::FindOrCreateConfigCache(const FString& InFilePath)
{
	if (!GConfig)
	{
		return nullptr;
	...
	const FString NormalizedFilePath = FConfigCacheIni::NormalizeConfigIniPath(InFilePath);
	FConfigFile* OutConfigFile = FindConfigCache(NormalizedFilePath);

	if (!OutConfigFile)
	{
		FConfigFile NewConfigFile;
		NewConfigFile.Read(NormalizedFilePath);
		OutConfigFile = &GConfig->Add(NormalizedFilePath, NewConfigFile);
	}
...
FConfigFile* UFileHelperBPLibrary::FindConfigCache(const FString& InFilePath)
{
	if (!GConfig)
	{
		return nullptr;
	...
	return GConfig->FindConfigFile(FConfigCacheIni::NormalizeConfigIniPath(InFilePath));
}

void UFileHelperBPLibrary::WriteConfigKey(const FString& InFilePath, const FString& InSection, const FString& InKey, bool& Success, bool SingleLineArrayWrite, const UStruct* Value)
{
	checkNoEntry();
}

bool UFileHelperBPLibrary::RemoveConfigKey(const FString& InFilePath, const FString& InSection, const FString& InKey)
{
	if (!GConfig)
	{
		return false;
	...
	if (FConfigFile* ConfigFile = FindConfigCache(InFilePath))
	{
		return ConfigFile->RemoveKeyFromSection(*InSection, *InKey);
	}

	return false;
}

bool UFileHelperBPLibrary::RemoveConfigSection(const FString& InFilePath, const FString& InSection)
{
	if (!GConfig)
	{
		return false;
	...
	FConfigFile* ConfigFile = FindConfigCache(InFilePath);
	if (!ConfigFile)
	{
		return false;
	}

	return ConfigFile->Remove(InSection) > 0;
}

bool UFileHelperBPLibrary::CreateConfig(const FString& InFilePath, bool bInTransient)
{
	if (!GConfig)
	{
		return false;
	...
	if (FConfigFile* ConfigFile = FindOrCreateConfigCache(InFilePath))
	{
		if (bInTransient)
		{
...
bool UFileHelperBPLibrary::ReloadConfig(const FString& InFilePath)
{
	if (!GConfig)
	{
		return false;
...
bool UFileHelperBPLibrary::SaveConfig(const FString& InFilePath)
{
	if (!GConfig)
	{
		return false;
	...
	if (FConfigFile* ConfigFile = FindConfigCache(InFilePath))
	{
		return ConfigFile->Write(InFilePath);
	}

	return false;
}

bool UFileHelperBPLibrary::ClearConfig(const FString& InFilePath)
{
	if (!GConfig)
	{
		return false;
	...
	const FString NormalizedFilePath = FConfigCacheIni::NormalizeConfigIniPath(InFilePath);
	if (!IsConfigLoaded(NormalizedFilePath))
	{
		return false;
	}

	GConfig->UnloadFile(NormalizedFilePath);

	return true;
}

bool UFileHelperBPLibrary::IsConfigLoaded(const FString& InFilePath)
{
	return GConfig != nullptr && FindConfigCache(InFilePath) != nullptr;
}

bool UFileHelperBPLibrary::IsConfigTransient(const FString& InFilePath)
{
	if (!GConfig)
	{
		return false;
	...
	if (const FConfigFile* ConfigFile = FindConfigCache(InFilePath))
	{
		return ConfigFile->NoSave;
	}
	
	return false;
}

bool UFileHelperBPLibrary::GetConfigSections(const FString& InFilePath, TArray<FString>& OutSections)
{
	const FString NormalizedFilePath = FConfigCacheIni::NormalizeConfigIniPath(InFilePath);
	if (FConfigFile* ConfigFile = FindConfigCache(NormalizedFilePath))
	{
		return GConfig->GetSectionNames(NormalizedFilePath, OutSections);
	}

	return false;
}

bool UFileHelperBPLibrary::GetConfigSectionKeys(const FString& InFilePath, const FString& InSection, TArray<FName>& OutKeys)
{
	if (const FConfigFile* ConfigFile = FindConfigCache(InFilePath))
	{
		if (const FConfigSection* ConfigSection = ConfigFile->FindSection(InSection))
		{
			return ConfigSection->GetKeys(OutKeys) > 0;
		}
	}
