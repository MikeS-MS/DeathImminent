...
UCLASS()
class FILEHELPER_API UFileHelperBPLibrary : public UBlueprintFunctionLibrary
{
	GENERATED_BODY()
...
public:
	...
	/** Reads a value at a specific key in a section from an existing config cache (no IO) */
	UFUNCTION(BlueprintCallable, Category = "FileHelper|Config", CustomThunk, meta = (CustomStructureParam = "OutValue"))
	static void ReadConfigKey(const FString& InFilePath, const FString& InSection, const FString& InKey, bool& Success, bool SingleLineArrayRead, UStruct*& OutValue);
	DECLARE_FUNCTION(execReadConfigKey)
	{
		...
		Success = UFileHelperBPLibrary::ReadConfigCache(FilePath, Section, Key, Property, ValuePtr, SingleLineArrayRead);
	}

	/** Writes a value at a specific key in a section from an existing config cache (no IO) */
	UFUNCTION(BlueprintCallable, Category = "FileHelper|Config", CustomThunk, meta = (CustomStructureParam = "Value"))
	static void WriteConfigKey(const FString& InFilePath, const FString& InSection, const FString& InKey, bool& Success, bool SingleLineArrayWrite, const UStruct* Value);
	DECLARE_FUNCTION(execWriteConfigKey)
	{
		P_GET_PROPERTY(FStrProperty, FilePath);
		P_GET_PROPERTY(FStrProperty, Section);
		P_GET_PROPERTY(FStrProperty, Key);
		...
		Success = UFileHelperBPLibrary::WriteConfigCache(FilePath, Section, Key, Property, ValuePtr, SingleLineArrayWrite);
	}

	/** Removes a specific key in a section from an existing config cache (no IO) */
	UFUNCTION(BlueprintCallable, Category = "FileHelper|Config")
	static bool RemoveConfigKey(const FString& InFilePath, const FString& InSection, const FString& InKey);

	/** Removes a specific section from an existing config cache (no IO) */
	UFUNCTION(BlueprintCallable, Category = "FileHelper|Config")
	static bool RemoveConfigSection(const FString& InFilePath, const FString& InSection);

	/** 
	 * Creates a config cache based on a filepath, 
	 * if cache already exists, nothing will happen
	 * if file exists on disk it will be loaded into cache
	 * Transient will not create/save any file when SaveConfig is called or during engine shutdown
	 */
	UFUNCTION(BlueprintCallable, Category = "FileHelper|Config")
	static bool CreateConfig(const FString& InFilePath, bool bInTransient = false);
	
	/** Reloads existing config cache from disk again, config cache should exist first */
	UFUNCTION(BlueprintCallable, Category = "FileHelper|Config")
	static bool ReloadConfig(const FString& InFilePath);

	/** Saves existing config from cache to disk, instead of waiting engine shutdown to do so */
	UFUNCTION(BlueprintCallable, Category = "FileHelper|Config")
	static bool SaveConfig(const FString& InFilePath);

	/** Clears an existing config cache, data can be lost if cache was not written to disk first (no IO) */
	UFUNCTION(BlueprintCallable, Category = "FileHelper|Config")
	static bool ClearConfig(const FString& InFilePath);

	/** Checks whether a file was loaded into config cache (no IO) */
	UFUNCTION(BlueprintCallable, Category = "FileHelper|Config")
	static bool IsConfigLoaded(const FString& InFilePath);
	
	/** Checks whether a config cache is marked as transient and will not get saved (no IO) */
	UFUNCTION(BlueprintCallable, Category = "FileHelper|Config")
	static bool IsConfigTransient(const FString& InFilePath);

	/** Get all sections from an exiting config cache (no IO) */
	UFUNCTION(BlueprintCallable, Category = "FileHelper|Config")
	static bool GetConfigSections(const FString& InFilePath, TArray<FString>& OutSections);

	/** Get all keys from an exiting config cache section (no IO) */
	UFUNCTION(BlueprintCallable, Category = "FileHelper|Config")
	static bool GetConfigSectionKeys(const FString& InFilePath, const FString& InSection, TArray<FName>& OutKeys);
...
protected:
	...
	/* CONFIG */
	static bool WriteConfigCache(const FString& Filename, const FString& Section, const FString& Key, FProperty* Type, void* Value, bool SingleLineArray);
	static bool ReadConfigCache(const FString& Filename, const FString& Section, const FString& Key, FProperty* Type, void* Value, bool SingleLineArray);
	static FConfigFile* FindOrCreateConfigCache(const FString& InFilePath);
	static FConfigFile* FindConfigCache(const FString& InFilePath);
	// datatable csv
